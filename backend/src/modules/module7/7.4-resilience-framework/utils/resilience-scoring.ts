...
export function weightedResilienceScore(m: Omit<ResilienceMetrics, 'overallScore'>): number {
  ...
  return pct(Math.pow(s, 1 / (WEIGHTS.criticalPath + WEIGHTS.resource + WEIGHTS.dependency + WEIGHTS.buffer)) * 100);
}
