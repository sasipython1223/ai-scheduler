...
export class RobustnessCalculator {
  ...
  resourceRisk(r: ResourceStats): number {
    ...
    const volatility = pct(
      100 * Math.min(1, r.stdevUtil / Math.max(1e-6, r.meanUtil)),
    );
    const headroom = inverse(pct(100 * Math.max(0, 1 - r.maxUtil))); // less headroom => more risk
    return pct(0.45 * overload + 0.35 * volatility + 0.2 * headroom);
  }
  ...
  assemble(
    metrics: Omit<ResilienceMetrics, "overallScore">,
  ): ResilienceMetrics {
