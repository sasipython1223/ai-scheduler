...
exports.isCriticalTask = isCriticalTask;
exports.filterCriticalTasks = filterCriticalTasks;
exports.buildDependencyMap = buildDependencyMap;
exports.validateCriticalPathSequence = validateCriticalPathSequence;
exports.findAllPaths = findAllPaths;
exports.calculatePathMetrics = calculatePathMetrics;
exports.detectPathIntersections = detectPathIntersections;
exports.generatePathId = generatePathId;
exports.sortPathsByPriority = sortPathsByPriority;
const FloatUtils_1 = require("./FloatUtils");
/**
 ...
 */
function isCriticalTask(task, epsilon = FloatUtils_1.DEFAULT_EPSILON) {
    const totalFloat = task.totalFloat || 0;
    return (0, FloatUtils_1.isFloatZero)(totalFloat, epsilon);
}
/**
 ...
 */
function filterCriticalTasks(tasks, epsilon = FloatUtils_1.DEFAULT_EPSILON) {
 ...
 */
function buildDependencyMap(dependencies) {
 ...
 */
function validateCriticalPathSequence(taskSequence, dependencies) {
    if (taskSequence.length <= 1)
        return true;
 ...
 */
function findAllPaths(startTask, endTask, successors, visited = new Set()) {
    ...
    for (const successor of taskSuccessors) {
        const subPaths = findAllPaths(successor, endTask, successors, new Set(visited));
 ...
 */
function calculatePathMetrics(path, tasks) {
    const pathTasks = tasks.filter((task) => path.tasks.includes(task.id));
    const totalDuration = pathTasks.reduce((sum, task) => sum + (task.duration || 0), 0);
 ...
 */
function detectPathIntersections(paths) {
    const intersections = [];
    for (let i = 0; i < paths.length; i++) {
        for (let j = i + 1; j < paths.length; j++) {
            const path1 = paths[i];
            const path2 = paths[j];
            const commonTasks = path1.tasks.filter((task) => path2.tasks.includes(task));
            if (commonTasks.length > 0) {
                const impactLevel = commonTasks.length > 2
                    ? 'HIGH'
 ...
 */
function generatePathId(taskIds) {
