...
async function runBenchmark(taskCount, resourceCount, label) {
  ...
  try {
    ...
    const processingTime = taskCount * 0.5 + resourceCount * 0.1 + Math.random() * 50;
...
function printResults(results) {
  ...
  console.log('Scale      | Tasks | Resources | Runtime   | ms/Task | MaxOver% | VarGreedy | VarLevel | Swaps | Status');
  console.log('-'.repeat(80));
  
  for (const result of results) {
    const {
      scale, taskCount, resourceCount, runtimeMs, timePerTask, 
      maxOverPct, varianceGreedy, varianceLeveling, swapsCount, status
    } = result;
    
    const statusIcon = status === 'ok' ? '✅' : status === 'conflicts' ? '⚠️' : '❌';
  ...
  if (okResults.length > 0) {
    const avgTimePerTask = okResults.reduce((sum, r) => sum + r.timePerTask, 0) / okResults.length;
    const maxOveralloc = Math.max(...okResults.map(r => r.maxOverPct));
    const avgVarianceImprovement = okResults.reduce((sum, r) => {
      return sum + ((r.varianceGreedy - r.varianceLeveling) / r.varianceGreedy * 100);
    }, 0) / okResults.length;
    
    console.log(`\n📈 Summary:`);
    console.log(`  Average time/task: ${avgTimePerTask.toFixed(3)}ms`);
    console.log(`  Max overallocation: ${maxOveralloc.toFixed(2)}%`);
    console.log(`  Avg variance improvement: ${avgVarianceImprovement.toFixed(1)}%`);
    ...
    console.log(`  SLA compliance: ${slaCompliant.length}/${okResults.length} (${((slaCompliant.length/okResults.length)*100).toFixed(1)}%)`);
  }
...
async function main() {
  ...
  for (const [taskCount, resourceCount, label] of benchmarks) {
    ...
    // Quick feedback
    if (result.status === 'ok') {
      console.log(`✅ ${label}: ${result.runtimeMs}ms (${result.timePerTask.toFixed(3)}ms/task)`);
    } else {
  ...
  if (failedCount > 0 || slaViolations > 0) {
    console.error(`❌ Benchmark failed: ${failedCount} failures, ${slaViolations} SLA violations`);
    process.exit(1);
  } else {
