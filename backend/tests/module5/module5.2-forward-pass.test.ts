...
import {
  ...
  computeForwardPass,
} from '../../src/modules/module5/module5.2-forward-pass/forward-pass-engine';
import { calculateProjectEndDate } from '../../src/modules/module5/module5.2-forward-pass/forward-pass-utils';
import {
  LogicLink,
  LogicType,
  TaskInput,
...
describe('Module 5.2: CPM Forward Pass', () => {
  ...
  describe('ForwardPassCalculator', () => {
    ...
    it('should handle simple linear dependency chain (A → B → C)', () => {
      ...
      expect(new Date(taskC.earlyStart).getTime()).toBeGreaterThanOrEqual(
  ...
  describe('Utility Functions', () => {
    ...
    it('should calculate project end date using utility function', () => {
      ...
      // Run forward pass first to get scheduled tasks
      const forwardPassResult = calculator.computeForwardPass(tasks, links);
      const endDate = calculateProjectEndDate(forwardPassResult.tasks);
